...
const copyHtml = async ({ mode, watchEvent, watchPath }) => {
  ...
  const copyHtmlFile = async ({ copyFilePath }) => {
    ...
    try {
      ...
      await console.error(
        `Error in ${chalk.underline('copyHtmlFile')}.: ${chalk.bold.italic.bgRed(error.name)} ${chalk.red(
          error.message,
        )}`,
      );
    }
  ...
  try {
    ...
    await console.error(
      `Error in ${chalk.underline('copyHtml')}.: ${chalk.bold.italic.bgRed(error.name)} ${chalk.red(error.message)}`,
    );
  }
};
